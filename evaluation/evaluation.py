...
def eval_edge_prediction(model, data, n_neighbors, batch_size=200, vs=None, rs=None):
  ...
  with torch.no_grad():
    ...
    for k in range(num_test_batch-1):
      ...
      # sources_batch_neg, destinations_batch_neg= negative_edge_sampler.sample(edges)
...
def eval_edge_prediction2(model, negative_edge_sampler, data, n_neighbors, batch_size=200, vs=None, rs=None):
  # Ensures the random sampler uses a seed for evaluation (i.e. we sample always the same
  # negatives for validation / test set)
  ...
  with torch.no_grad():
    ...
    for k in range(num_test_batch-1):
      ...
      pos_prob, neg_prob = model.compute_edge_probabilities(sources_batch, destinations_batch,
                                                            sources_batch_neg, destinations_batch_neg,
