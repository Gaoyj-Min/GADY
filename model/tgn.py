...
class TGN(torch.nn.Module):
  def __init__(self, neighbor_finder, node_features, edge_features, device, n_layers=2,
    ...
    self.embedding_module = get_embedding_module(module_type="PINT",
  ...
  def compute_temporal_embeddings(self, source_nodes, destination_nodes, edge_times,
    ...
    node_embedding = self.embedding_module.compute_embedding(memory=memory,
                                                             ...
                                                             time_diffs=time_diffs, V=self.V, R=self.R, targets=targets,
  ...
  def compute_edge_probabilities(self, source_nodes, destination_nodes, edge_times,
    ...
    #   source_nodes, destination_nodes, negative_nodes, edge_times, edge_idxs, n_neighbors, nextR=nextR, nextV=nextV)

    # source_node_embedding, destination_node_embedding,\
    #   source_node_embedding_neg, destination_node_embedding_neg = self.compute_temporal_embeddings(
    #   source_nodes, destination_nodes,source_nodes_neg,destination_nodes_neg, edge_times, edge_idxs, n_neighbors)
      
    # neg_source_embedding, negative_node_embedding = self.compute_temporal_embeddings(source_nodes_neg, destination_nodes_neg,
    ...
    source_embedding, destination_embedding = self.compute_temporal_embeddings(source_nodes, destination_nodes,
                                                                                        edge_times, edge_idxs,
                                                                                        n_neighbors,persist=update_memory)

    # score = self.affinity_score(torch.cat([source_embedding, destination_embedding], dim=1))
  ...
  def compute_edge_probabilities2(self, source_nodes, destination_nodes, source_nodes_neg,destination_nodes_neg, edge_times, edge_time_neg,
                                 edge_idxs, n_neighbors=20, next_R=None, next_V=None):
    """
    Compute probabilities for edges between sources and destination and between sources and
    negatives by first computing temporal embeddings using the TGN encoder and then feeding them
    into the MLP decoder.
    ...
    neg_source_embedding, negative_node_embedding = self.compute_temporal_embeddings(source_nodes_neg, destination_nodes_neg,
                                                                                     edge_time_neg, edge_idxs, n_neighbors,
                                                                                     persist=False)
    pos_source_embedding, destination_node_embedding = self.compute_temporal_embeddings(source_nodes, destination_nodes,
                                                                                        edge_times, edge_idxs,
                                                                                        n_neighbors)

    score = self.affinity_score(torch.cat([pos_source_embedding, neg_source_embedding], dim=0),
